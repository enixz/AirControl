...
from orchestrator import AirControlOrchestrator, choose_camera_device
...
class TestFindAvailableCameras(unittest.TestCase):
    ...
    def test_returns_available_list(self):
        """返回可用摄像头列表"""
        with patch('orchestrator.list_available_cameras', return_value=[
            {"index": 0, "name": "摄像头 0"},
            {"index": 2, "name": "摄像头 2"},
        ]):
            result = AirControlOrchestrator._find_available_cameras(
                None,
                exclude_index=1,
            )
    ...
    def test_exception_returns_empty(self):
        """枚举异常时返回空列表不崩溃"""
        with patch('orchestrator.list_available_cameras', side_effect=RuntimeError("boom")):
            result = AirControlOrchestrator._find_available_cameras(
                None,
                exclude_index=1,
            )
    ...
    def test_startup_probe_does_not_exclude_unverified_preference(self):
        with patch(
            "orchestrator.list_available_cameras",
            return_value=[],
        ) as probe:
            AirControlOrchestrator._find_available_cameras(None)
        probe.assert_called_once_with(max_probe=4, exclude_index=None)
...
class TestChooseCameraIndex(unittest.TestCase):
    ...
    def test_use_requested_when_available(self):
        ...
        idx, backend, fallback = choose_camera_device(1, available)
    ...
    def test_fallback_to_first_when_requested_unavailable(self):
        """偏好索引不可用时回退到第一个可用索引"""
        available = [{"index": 0, "backend": 123}]
        idx, backend, fallback = choose_camera_device(1, available)
        self.assertEqual(idx, 0)
        self.assertEqual(backend, 123)
        self.assertTrue(fallback)

    def test_use_requested_when_no_camera_found(self):
        """探测失败时仍尝试使用偏好索引"""
        idx, backend, fallback = choose_camera_device(1, [])
        self.assertEqual(idx, 1)
        self.assertIsNone(backend)
        self.assertFalse(fallback)
    ...
    def test_config_index_zero_fallback_works(self):
        ...
        idx, _, fallback = choose_camera_device(0, available)
        self.assertEqual(idx, 1)
        self.assertTrue(fallback)
