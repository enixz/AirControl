...
通过 **单手连续两次“抓取与释放”**（握拳->张开->握拳->张开）手势循环切换模式：
...
| ✊🖐️✊🖐️ 单手连续两次“抓取与释放” | 切换交互模式（演示↔鼠标↔板书） |

**智能形状校正**：开启后，手绘的线条、三角形、矩形、椭圆会自动修正为标准几何图形。

> 💡 **单手连续抓取切模式** 是全局通用的，不只板书模式。任何模式下单手快速完成“握拳->张开->握拳->张开”动作（2.5秒内）即触发。
...
| 指令 | 动作 |
|------|------|
| `演示模式` `鼠标模式` `板书模式` | 直接跳到指定模式（无需单手连续抓取切换） |
...
│   ├── mode_manager.py            # 模式管理器（单手连续抓取切换）
